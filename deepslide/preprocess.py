...
def is_purple(crop: np.ndarray, purple_threshold: int,
    ...
    rgb_cond1 = r > g - 10
    rgb_cond2 = b > g - 10
    rgb_cond3 = ((r + b) / 2) > g + 20

    hsv_pooled = rgb2hsv(pooled/255)
    h, s, v = (1 - hsv_pooled[:, :, 0])*360, hsv_pooled[:, :, 1]*100, hsv_pooled[:, :, 2]*100
    hsv_cond1 = h > 100
    hsv_cond2 = v > 70
    hsv_cond3 = s > 50
    
    # Find the indexes of pooled satisfying all 3 conditions.
    pooled = pooled[(rgb_cond1 & rgb_cond2 & rgb_cond3) | (hsv_cond1 & hsv_cond2 & hsv_cond3)]
...
def get_subfolder_to_overlap(subfolders: List[Path],
    ...
    for subfolder in subfolders:
        ...
        overlap_factor = max(
            1.0,
            math.pow(
                math.sqrt(desired_crops_per_class / (subfolder_size / 6)),
                1.5))
